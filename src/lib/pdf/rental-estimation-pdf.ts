...
export type RentalEstimationPdfResult = PdfResult;

/**
 * Rental Estimation PDF - generated for active/unreturned rentals before any
 * items have been returned. Mirrors the original "Rental Estimation" layout:
 * title, "Estimation #", Start Date, and an End Date that falls back to N/A
 * when the rental hasn't concluded yet.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function generateRentalEstimationPdf(billing: any): Promise<RentalEstimationPdfResult> {
  return new Promise((resolve, reject) => {
    try {
      const docTitle = 'RENTAL ESTIMATION';
      ...
      doc.on('end', () => {
        ...
        const filename = `${customerName}_${dateStr}_Estimate.pdf`;
      ...
      doc.text(`Estimation #${billing.id}`, rightColX, MARGIN + 22, { lineBreak: false });
      ...
      else doc.text(`End Date: N/A`, rightColX, MARGIN + 82, { lineBreak: false });
      ...
      const totalDue = rentalCharges + labourCost + transportCost;
